...
static int MEM_stackBounds(const pthread_t id, mem_allocator_t *const allocator)
{
    ...
    if (grows_down == true)
    {
        allocator->stack_bottom =
            (uintptr_t *)((char *)base_addr + guard_size);
        allocator->stack_top =
            (uintptr_t *)((char *)base_addr + stack_size);
    }
    else
    {
        allocator->stack_bottom =
...
void *MEM_sbrk(const intptr_t increment)
{
    ...
    if (sbrk(increment) == (void *)-1) 
    {
        ...
        LOG_ERROR("Failed to adjust break (increment=%" PRIdPTR ""
                    "Error code: %d.\n",
                    increment, (int)(intptr_t)old_break);
        goto function_output;
    }
                                ...
                                const size_t size)
{
    ...
    if (UNLIKELY(index >= allocator->num_size_classes))
    {
        LOG_WARNING("Size overflow - "
                    ...
                    "Clamped to class %zu.\n",
                    size, max_class, (allocator->num_size_classes - 1u));
                                ...
                                const intptr_t inc)
{
    ...
    if (UNLIKELY(allocator == NULL))
    {
        ...
        LOG_ERROR("Invalid arguments: allocator=%p. "
                  "Error code: %d.\n",
                  (void *)allocator, (int)(intptr_t)old);
        goto function_output;
    }
...
static int *MEM_mapAlloc(mem_allocator_t *allocator, const size_t total_size)
{
    ...
    if (UNLIKELY(allocator == NULL))
    {
        ...
        LOG_ERROR("Invalid arguments: allocator=%p. "
                  "Error code: %d.\n",
                  (void *)allocator, (int)(intptr_t)ptr);
        goto function_output;
    }
    ...
    if (map_block == NULL)
    {
        ...
        LOG_ERROR("Mmap metadata allocation failed: %zu bytes. "
                    "Error code: %d.\n",
                    sizeof(mmap_t), (int)(intptr_t)ptr);
        goto function_output;
    }
            ...
            *map_ref = to_free->next;
            ...
            if (ret != EXIT_SUCCESS)
            {
                LOG_ERROR("Mmap metadata free failed: %zu bytes. "
                            "Error code: %d.\n",
                            sizeof(mmap_t), (int)(intptr_t)ret);
                goto function_output;
            }
                            ...
                            block_header_t **fit_block)
{
    ...
    for (class_idx = (size_t)start_class;
        class_idx < allocator->num_size_classes;
        class_idx++)
    {
    ...
    LOG_WARNING("First-fit allocation failed: "
                ...
                "Error code: %d.\n",
                size, (size_t)(allocator->num_size_classes - 1u), ret);
    ...
    if (size > MMAP_THRESHOLD)
    {
        ...
        if (raw_mmap == NULL)
        {
            ...
            LOG_ERROR("Mmap failed: grow heap by %zu bytes. "
                        "Error code: %d.\n",
                        total_size, (int)(intptr_t)user_ptr);
            goto function_output;
        }
    ...
    if (ret == -ENOMEM)
    {
        ...
        if ((intptr_t)old_brk < 0)
        {
            ...
            LOG_ERROR("Heap grow failed: requested %zu bytes, old_brk=%ld. "
                        "Error code: %d.\n",
                        total_size, (long)old_brk, (int)(intptr_t)user_ptr);
            goto function_output;
        }
            ...
            if (block_addr >= heap_start && block_addr < heap_end) 
            {
                    ...
                    ret == EXIT_SUCCESS && !block->free)
                {
                    ...
                    LOG_INFO("Block Marked(sbrk): %p (%zu bytes). "
                            "stack = %lu.\n",
                            (void *)((uint8_t*)block + sizeof(block_header_t)),
                            block->size, (unsigned long)stack_ptr);
                }
                else
                {
                    block->marked = 0u;
...
static int MEM_gcSweep(mem_allocator_t *const allocator)
{
    ...
    heap_start = (uint8_t *)((uintptr_t)allocator->heap_start +
                                            allocator->metadata_size);
    ...
    while (*scan)
    {
        ...
        LOG_INFO("Block Sweep(mmap): block %p (%zu bytes). "
                    "free=%u | marked=%u.\n",
                    (void*)((uint8_t*)block + sizeof(block_header_t)),
                    map->size, block->free, block->marked);
...
static int MEM_runGc(mem_allocator_t *const allocator)
{
    ...
    if (!gc_thread->gc_thread_started)
    {
        ...
        ret = pthread_create(&gc_thread->gc_thread, NULL, MEM_gcThreadFunc,
                                allocator);
        if (ret != EXIT_SUCCESS)
        {
            LOG_ERROR("Failed to create gc thread: %p. "
                  "Error code: %d.\n",
                  (void *)gc_thread->gc_thread, ret);
            goto function_output;
        }
    }
    else
...
void *MEM_allocMallocFirstFit(mem_allocator_t *const allocator,
                                const size_t size, const char *const var)
{
    void *ret = NULL;
    ...
    pthread_mutex_lock(&gc_thread->gc_lock);
    ret = MEM_allocatorMalloc(allocator, size, __FILE__,
                        ...
                        const allocation_strategy_t strategy)
{
    ...
    pthread_mutex_lock(&gc_thread->gc_lock);
    ret = MEM_allocatorCalloc(allocator, size, __FILE__, __LINE__,
                                var, strategy);
    pthread_mutex_unlock(&gc_thread->gc_lock);
...
void *MEM_allocRealloc(mem_allocator_t *const allocator,
                        void *const ptr, const size_t new_size,
                        const char *const var,
                        const allocation_strategy_t strategy)
{
    void *ret = NULL;
    ...
    pthread_mutex_lock(&gc_thread->gc_lock);
    ret = MEM_allocatorRealloc(allocator, ptr, new_size, __FILE__,
                                __LINE__, var, strategy);
    pthread_mutex_unlock(&gc_thread->gc_lock);
 ...
 *  @note       Automatically passes file and line information.
 * ========================================================================== */
int MEM_allocFree(mem_allocator_t *const allocator, void *const ptr,
                    const char *const var)
{
    int ret = EXIT_SUCCESS;
