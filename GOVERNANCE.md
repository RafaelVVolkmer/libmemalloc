...
<img
    src="/assets/images/libmemalloc_governance.svg"
...
technical discussion
    -> rough consensus
        -> maintainer decision when needed
            -> scientific record for each engineering decision
```
...
Supporting discussion and evidence may be recorded in:
...
Record the rationale and reviewed revision in the issue or pull request.
Link supporting discussions and restricted evidence without publishing sensitive content.
...
Use the issue or pull request to record the problem, chosen approach, alternatives, compatibility effects
and verification evidence. Follow the [engineering review procedure](docs/procedures/decision-making.md).
Keep stable requirements and executable contracts in the SDDs and reference pages they govern.
The reviewer assesses the evidence and the decision authority records acceptance for the reviewed revision.
...
[`.github/CODEOWNERS`](.github/CODEOWNERS) is the canonical path ownership map.
It assigns Rafael V. Volkmer to the current repository areas and provides a
fallback owner for new paths.

The map covers product sources (`core/`), build configuration (`cmake/`),
packaging, automation, infrastructure, tooling, tests, documentation,
Doxygen sources, visual assets, GitHub metadata, contributor settings,
license texts, and root policy documents. Patterns start at the repository
root; later matching rules take precedence.

GitHub uses the ownership map to request reviews. Required owner approval
must also be enabled in branch protection rules or rulesets.
...
- update the ownership map when repository areas or maintainers change.
...
The Release Owner records the release decision and its evidence under the
[release procedure](docs/procedures/release.md).
...
| Release authority unclear    | Releases become inconsistent or risky                | Define Release Owner and release evidence record                        |
...
[rafaelvvolkmer-github-url]: https://github.com/RafaelVVolkmer

Maintainers own approval of [specification issues](docs/procedures/specifications.md) and the reserved
`spec` and SDD area labels. The [versioned roster and catalog](.github/specifications.lua) require review
alongside changes to maintainer responsibility. PRs close bounded issues after their acceptance criteria pass;
partial implementation references the open issue.
