...
<img
    src="/assets/images/libmemalloc_maintainers.svg"
...
The canonical path ownership map is [`.github/CODEOWNERS`](.github/CODEOWNERS).
All current areas are assigned to `@RafaelVVolkmer`, including product sources,
build and packaging, automation and tests, documentation and assets,
repository settings, licensing, and community policies.

Keep path assignments in that file when the repository structure changes.
GitHub requests reviews using the map from the pull request's base branch.
Required owner approval also depends on branch protection rules or rulesets.
